...
# ── Calcolo EV e Kelly Criterion ──────────────────────────────
def calcola_ev(prob, quota):
    """Expected Value: quanto guadagni in media per ogni euro scommesso"""
    try:
        prob = float(prob) / 100
        quota = float(quota)
        ev = (prob * (quota - 1)) - (1 - prob)
        return round(ev * 100, 2)  # In percentuale
    except:
        return None

def calcola_kelly(prob, quota, bankroll=BANKROLL, frazione=0.25):
    """Kelly Criterion frazionato — suggerisce quanto puntare"""
    try:
        prob = float(prob) / 100
        quota = float(quota)
        kelly = (prob * quota - 1) / (quota - 1)
        kelly_frazionato = kelly * frazione  # Quarto Kelly per sicurezza
                        ...
                        if market.get("key") == "h2h":
                            ...
                            for o in outcomes:
                                ...
                                if name == event.get("home_team"):
                                    if price > best["1"][0]:
                                        best["1"] = (price, bk_name)
                                elif name == "Draw":
                                    if price > best["X"][0]:
                                        best["X"] = (price, bk_name)
                                elif name == event.get("away_team"):
                                    if price > best["2"][0]:
                                        best["2"] = (price, bk_name)
                return best
    except Exception as e:
        print(f"Errore confronto quote: {e}")
    return {}
...
QUOTE REALI BOOKMAKER: {odds[:3] if odds else 'non disponibili'}

Usa le quote reali. NON inventare quote. Se non disponibili scrivi N/D.
...
Rispondi SOLO in JSON senza backtick:
giocata_consigliata (1 X o 2), quota_live,
over_under_live, quota_over_under_live,
gol_no_gol_live, quota_gol_no_gol_live,
...
def format_message(match, analysis, best_odds=None):
    ...
    # Confronto quote migliori
    best_str = ""
    if best_odds:
        best_str = "\n\U0001f4b0 <b>Migliori quote sul mercato:</b>\n"
...
def format_live_message(match, analysis):
    ...
    return (
        ...
        f"\U0001f4ca Over/Under: {a.get('over_under_live','N/A')} @ {a.get('quota_over_under_live','?')}\n"
        f"\u26bd Gol/No Gol: {a.get('gol_no_gol_live','N/A')} @ {a.get('quota_gol_no_gol_live','?')}\n"
...
def multipla_job():
    ...
    msg += f"\U0001f4b0 <b>Quota combinata stimata: {round(quota_combined, 2)}</b>\n"
    if kelly_pct:
        msg += f"\U0001f3b2 Kelly consigliato: {kelly_pct}% del bankroll (€{kelly_eur})\n"
    send_telegram(msg)
