...
def format_prematch(m, a_raw, odds):
    ...
    return (
        ...
        f"\U0001f4a1 <b>{a.get('value_bet','?')}</b> @ {a.get('quota','?')} | O/U: {a.get('over_under','?')} @ {a.get('quota_ou','?')}\n"
        f"\U0001f525 Confidence: {conf}/100\n"
        f"{odds_str}"
...
def format_live(m, a_raw):
    ...
    return (
        ...
        f"\U0001f3b0 <b>{a.get('giocata','N/A')}</b> @ {a.get('quota','?')}\n"
        f"{rischio_emoji} Rischio: {rischio} | \U0001f525 {a.get('confidence','?')}/100\n"
        f"\U0001f4dd {a.get('motivazione','')}\n"
    )
